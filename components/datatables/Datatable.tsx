...
const ComponentsDatatablesRangeSearch: React.FC<ComponentsDatatablesRangeSearchProps> = ({ data, columns, minAgeFilter, maxAgeFilter }) => {
    ...
    useEffect(() => {
        ...
        if (minAgeFilter && minAge !== '' && minAge !== null) {
            filteredData = filteredData.filter((d) => {
                const value = d[minAgeFilter];
                return typeof value === 'number' && value >= Number(minAge);
            });
        }
        if (maxAgeFilter && maxAge !== '' && maxAge !== null) {
            filteredData = filteredData.filter((d) => {
                const value = d[maxAgeFilter];
                return typeof value === 'number' && value <= Number(maxAge);
            });
        }
        if (minAge || maxAge) {
